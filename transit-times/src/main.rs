...
fn read() -> Result<(Stops, Routes), Box<Error>> {
    print!("Reading...");
    let mut stops = HashMap::new();
    for result in csv::Reader::from_path("stops.txt")?.into_deserialize() {
        let result: RawStop = result?;
        stops.insert(StopId(result.stop_id), Stop::new(result));
    }
    let mut stop_times = Vec::new();
    for result in csv::Reader::from_path("stop_times.txt")?.into_deserialize() {
        stop_times.push(result?);
    }
    println!(" done.");
    let routes = build_routes(&stop_times);
    let stops = Stops::new(stops);
    let mut transfers = build_transfers(&stops);
    // the iterator (routes.routes) takes priority over existing items
    transfers.routes.extend(routes.routes);
    Ok((stops, transfers))
}
